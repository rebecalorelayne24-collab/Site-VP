...
def extrair_valor_pdf_com_ia(arquivo_pdf):
    ...
    try:
        ...
        # 🔑 Sua API KEY oficial do Google AI Studio configurada direto no sistema

        SUA_API_KEY = "AQ.Ab8RN6IXWZPhjzDLfvRwA2bSzbN7U8FXuXSB_eel7xgazpU8EA"

        

        # Inicialização moderna da biblioteca google-genai
        ...
        # Nova estrutura de chamada unificada para geração de conteúdo
...
def obter_texto_parcelamento(serviço, valor_total):

    """Aplica as regras da planilha e já calcula o valor matemático de cada parcela"""

    if serviço == "Revisão Bibliográfica":
    ...
    if serviço == "Rotulagem Nutricional":
    ...
    elif serviço == "Tabela Nutricional":
        ...
        if valor_total < 200:
            ...
            texto_especial = "Parcelamento Especial (Olhar com Presidência/Diretoria)"
...
def gerar_docx_proposta(dados):
    ...
    data_validade = calcular_data_final_uteis(datetime.now(), 15).strftime("%d/%m/%Y")
    ...
    if dados["tipo_servico"] == "Serviço Autoral (Farmácia Jr.)":
        ...
        hdr[0].text, hdr[1].text, hdr[2].text, hdr[3].text = 'Serviços', 'Valor unitário', 'Quantidade proposta', 'Total'
        ...
        doc.add_paragraph(f"\nPrazo de execução: {dados['prazo']} dias úteis (Previsão de entrega: {dados['data_entrega_autoral']})")

        doc.add_paragraph(f"Formas de pagamento: {dados['parcelas_cheio']}")

        

        doc.add_paragraph().add_run(f"\n2° opção – desconto de acordo [{dados['motivo_desconto']}]").bold = True
        ...
        hdr2[0].text, hdr2[1].text, hdr2[2].text, hdr2[3].text = 'Serviços', 'Valor unitário', 'Quantidade proposta', 'Total'
        ...
        hdr3[0].text, hdr3[1].text, hdr3[2].text, hdr3[3].text = 'Serviços', 'Valor unitário', 'Quantidade proposta', 'Total'
...
def renderizar_aba_precificacao():
    ...
    nome_lead = st.text_input("Nome do Lead / Empresa:", placeholder="Ex: Mika Doces Artesanais")

    tipo_servico = st.selectbox("Modalidade do Serviço:", ["Serviço Autoral (Farmácia Jr.)", "Serviço Terceirizado (Laboratório)"])
    ...
    if tipo_servico == "Serviço Autoral (Farmácia Jr.)":
        ...
        with col1:
            ...
            nome_servico = st.selectbox("Serviço Autoral:", list(PRECOS_AUTORAIS.keys()))

            valor_base = PRECOS_AUTORAIS[nome_servico]

            st.info(f"💰 Valor de tabela fixado pelo setor: **R$ {valor_base:,.2f}**")

            quantidade = st.number_input("Quantidade (Nº de Rótulos / Tabelas / Produtos):", min_value=1, value=1)
        ...
        data_entrega = calcular_data_final_uteis(datetime.now(), prazo).strftime("%d/%m/%Y")

        

        st.markdown("<h4 style='color: #FF1493;'>🔍 Seleção de Descontos (Limite Máximo de 15%)</h4>", unsafe_allow_html=True)
        ...
        st.markdown(f"**Soma dos descontos:** {soma_descontos:.2f}% | **Desconto real aplicado (Limite de 15%):** <span style='color:#FF1493; font-weight:bold;'>{desconto_final_pct:.2f}%</span>", unsafe_allow_html=True)
        ...
        dados_calculados = {
            ...
            "nome_lead": nome_lead, "tipo_servico": tipo_servico, "nome_servico": nome_servico,

            "valor_base": valor_base, "quantidade": quantidade, "total_cheio": total_cheio,

            "prazo": prazo, "data_entrega_autoral": data_entrega, "parcelas_cheio": parcelas_cheio, "parcelas_desconto": parcelas_desconto,

            "motivo_desconto": motivo_txt, "unitario_desconto": unitario_desconto, "total_desconto": total_desconto
        ...
        # --- MATEMÁTICA DA PLANILHA ATUALIZADA: =B4+110+0,18*(B4+110) ---
        ...
        data_entrega_terc = (datetime.now() + timedelta(days=prazo_final_terc)).strftime("%d/%m/%Y")

        

        # --- PARCELAMENTO DINÂMICO ADAPTADO PARA O TOTAL DO LABORATÓRIO ---

        parcelas_terceirizado = obter_texto_parcelamento("Tabela Nutricional", total_terceirizado)

        

        txt_coleta = f"R$ {valor_coleta:,.2f}" if "Oferecido" in coleta_opcao else "não é oferecido o serviço de coleta ficando a critério do cliente a disponibilização da amostra."
        ...
        dados_calculados = {
            ...
            "total_terceirizado": total_terceirizado, "prazo_terceirizado": prazo_final_terc, "data_entrega_terc": data_entrega_terc,

            "parametros": parametros, "metodologia": metodologia, "txt_coleta": txt_coleta, "parcelas_terceirizado": parcelas_terceirizado
...
Erro ao processar o PDF com a IA: module 'google.generativeai' has no attribute 'Client' 
