...
def renderizar_dashboard_geral():
    st.markdown(
        "<h2 style='text-align: center; color: #FF1493;'>📊 Dashboard"
        " Estratégico — Farmácia Jr.</h2>",
        unsafe_allow_html=True,
    )
    st.caption(
        "Visão consolidada da saúde financeira, origem de receitas e saídas por"
        " diretoria."
    )
    ...
    if df.empty:
        st.info(
            "ℹ️ O Dashboard será gerado assim que os primeiros lançamentos"
            " forem registrados no Fluxo de Caixa."
        )
        return

    # Tratamento de Tipos com Fallback de Segurança
    df["valor_liquido"] = pd.to_numeric(
        df["valor_liquido"], errors="coerce"
    ).fillna(0.0)
    df["data"] = pd.to_datetime(df["data"], errors="coerce")

    # Garantia de existência de colunas
    ...
    filtro_mes = col_f1.selectbox(
        "🗓️ Filtrar por Mês de Referência:", meses_ordem
    )
    ...
    if filtro_status == "Apenas Confirmados (Pagos)":
        df_filtrado = df_filtrado[
            df_filtrado["status_pagamento"].str.contains("Pago", na=False)
        ]
    ...
    m1.metric(
        ...
        f"R$ {receitas:,.2f}".replace(",", "X")
        .replace(".", ",")
        .replace("X", "."),
    )
    m2.metric(
        "📤 Despesas Acumuladas",
        f"R$ {despesas:,.2f}".replace(",", "X")
        .replace(".", ",")
        .replace("X", "."),
    )
    m3.metric(
    ...
    with col_esq:
        ...
        df_mensal = (
            df_filtrado.groupby(["mes", "tipo"])["valor_liquido"]
            .sum()
            .reset_index()
        )

        # Ordenação Cronológica Estrita
        df_mensal["mes"] = pd.Categorical(
            df_mensal["mes"], categories=meses_ordem[1:], ordered=True
        )
    ...
    with col_dir:
        ...
        if not df_rec.empty:
            fig_pizza = px.pie(
                ...
                hole=0.4,  # Transforma em Rosca para um visual mais moderno
            )
