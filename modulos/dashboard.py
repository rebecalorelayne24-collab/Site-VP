...
def renderizar_dashboard_geral():
    st.markdown("<h2 style='text-align: center; color: #FF1493;'>📊 Dashboard Estratégico — Farmácia Jr.</h2>", unsafe_allow_html=True)
    ...
    # =======================================================================
    meses_ordem = ["Todos", "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
    
    col_f1, col_f2 = st.columns(2)
    filtro_mes = col_f1.selectbox("🗓️ Filtrar por Mês de Referência:", meses_ordem)
    filtro_status = col_f2.radio("💰 Regime de Caixa:", ["Apenas Confirmados (Pagos)", "Todos (Incluir Pendentes)"], horizontal=True)
    ...
    if not df_desp.empty:
        fig_barras = px.bar(
            df_desp, x='departamento', y='valor_liquido', 
            color='categoria', barmode='stack',
            color_discrete_sequence=px.colors.qualitative.Safe,
            template="plotly_white",
            labels={'valor_liquido': 'Total Gasto (R$)', 'departamento': 'Diretoria', 'categoria': 'Categoria'}
        )
