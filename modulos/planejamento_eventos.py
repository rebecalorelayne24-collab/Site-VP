...
def conectar_banco_eventos():
    ...
    cursor = conn.cursor()
    # Cria a tabela de planejamento integrada se ela não existir
    cursor.execute('''
...
def renderizar_pagina_planejamento_eventos():
    ...
    evento_sel = st.selectbox("Selecione o Projeto para Gestão:", ["Dia do Açaí e Sundae (DDA)", "Treinamento de Imersão Interna", "Workshop de Rotulagem"])
    ...
    with st.expander("➕ Mapear Novo Custo / Aporte Planejado"):
        ...
        tipo = c3.selectbox("Tipo de Registro:", ["Previsão de Gasto", "Orçamento Alocado (Aporte)"])
        
        c4, c5, c6 = st.columns(3)
        item = c4.text_input("Insumo / Item Mapeado:")
        prio = c5.selectbox("Classificação de Prioridade:", ["essencial", "adicional", "NA"])
        valor = c6.number_input("Valor Estimado (R$):", min_value=0.0, value=10.0)
    ...
    conn = conectar_banco_eventos()
    df = pd.read_sql_query(f"SELECT id, responsavel, departamento, tipo_registro, prioridade, item, valor, status_compra FROM planejamento_eventos WHERE evento = '{evento_sel}'", conn)
    conn.close()
    
    if not df.empty:
        # Consolidação matemática automática
        aporte = df[df['tipo_registro'] == "Orçamento Alocado (Aporte)"]['valor'].sum()
        essencial = df[(df['tipo_registro'] == "Previsão de Gasto") & (df['prioridade'] == 'essencial')]['valor'].sum()
        adicional = df[(df['tipo_registro'] == "Previsão de Gasto") & (df['prioridade'] == 'adicional')]['valor'].sum()
        ...
        m4.metric("Margem Projetada", f"R$ {saldo_projetado:.2f}", delta=f"{saldo_projetado:.2f}")
        
        # Estilização visual Sinal de Trânsito para Prioridades
        ...
        sel_item = st.selectbox("Selecione o Insumo:", df['id'].astype(str) + " - " + df['item'])
        id_item = int(sel_item.split(" - ")[0])
        status_novo = st.selectbox("Fase Atual da Aquisição:", ["⚪ Planejado", "🟡 Em cotação", "🟢 Comprado", "🔵 Entregue no local"])
        
        if st.button("💾 Atualizar Etapa Logística", use_container_width=True, type="primary"):
            conn = conectar_banco_eventos()
            cursor = conn.cursor()
            cursor.execute("UPDATE planejamento_eventos SET status_compra = ? WHERE id = ?", (status_novo, id_item))
            conn.commit()
            conn.close()
